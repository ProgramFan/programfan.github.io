...
as a way to control temporarily the compiler behavior. By using `pragma GCC
optimize("O0")`, the optimization level can be set to zero, which means
absolutely no optimize for gcc.
